#include "mod_kdf740051yrt_kdf740051yrt.h"
#include <stdlib.h>
#include "ndarrays.h"
#include <stdint.h>
#include <stdio.h>
...
void matrix_prod(t_ndarray A, t_ndarray B, t_ndarray C, int64_t N, int64_t M, int64_t nb_threads)
{
    ...
    {
        #pragma omp for collapse(2) reduction(+: A) schedule(runtime)
        for (i = 0; i < M; i += 1)
        {
            for (j = 0; j < N; j += 1)
            {
        ...
        #pragma omp for collapse(2) reduction(+: B) schedule(runtime)
        for (i = 0; i < N; i += 1)
        {
            for (j = 0; j < M; j += 1)
            {
        ...
        /*Matrix Production*/
        #pragma omp for collapse(3) reduction(+: C) reduction(*: C) schedule(runtime)
        for (i = 0; i < M; i += 1)
        {
            for (j = 0; j < M; j += 1)
            {
